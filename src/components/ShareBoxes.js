  ...
  return (
    <div className="sbtci-box sbtci-state-share-box">
      <p className="sbtci-state-share-box-text">
        {props.social === 'twitter' ? props.children : 'Share this state\'s 2022 State Business Tax Climate rankings with your Facebook friends!'}
      </p>
...
const ShareBoxes = (props) => {
  const standardText = (stateData) => {
    return {
      text: `${stateData.name} ranks ${ordinal(stateData.total.rank)} on the @taxfoundation 2022 Index`,
  ...
  shareTexts.push({
    text: 'See how your state\'s tax code stacks up on @taxfoundation\'s 2022 State Business Tax Climate Index',
    hashtags: 'StateTaxReform'
  });
